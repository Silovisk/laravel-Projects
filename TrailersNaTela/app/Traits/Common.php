...
trait Common
{
    ...
    public function makePasswordHash($salt, $password)
    {
        if (strlen($password) < 6) {
            throw new Error('Password must be at least 6 characters long');
        }

        return Hash::make(substr($password, 0, 3) . $salt . substr($password, 3, strlen($password) - 3));
    }

    public function passwordIsValid(object $data)
    {
        $typedPassword = substr($data->typedPassword, 0, 3) . $data->salt . substr($data->typedPassword, 3, strlen($data->typedPassword) - 3);
        return Hash::check($typedPassword, $data->password);
    }
